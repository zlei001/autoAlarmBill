 ...
 *
 */
public class AlarmRecordLogMain {
	...
	public static void main(String[] args) throws UnknownHostException {
		logger = initLog(args,AlarmRecordLogMain.class);
	...
	public static void run() {
		AlarmLogRunningService delimitedLogRunningService = (AlarmLogRunningService)CattApplicationContextUtil.getBean("com.cattsoft.eomsteam.iot.alarm.exec.AlarmLogRunningService");
		while (true) {
			exec(delimitedLogRunningService);
			sleep();
		}
	}
	 ...
	 */
	public static void exec(AlarmLogRunningService delimitedLogRunningService) {
		try {
			String startTime = DateUtil.getNowTime();
			delimitedLogRunningService.doPending();
	...
	private static Log initLog(String[] args, Class clazz) {
		String lsClient = "alarmLog";
		if(args.length >0){
			if(DefaultStringUtil.isNotNull(args[0])){
				 lsClient = args[0];
			}
		}
		System.setProperty("alarmLog",lsClient);
