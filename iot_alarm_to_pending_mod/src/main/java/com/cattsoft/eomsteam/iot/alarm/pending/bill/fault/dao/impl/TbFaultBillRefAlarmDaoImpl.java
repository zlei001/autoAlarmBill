...
import com.cattsoft.eomsteam.uc.pub.db.QuoteBean;
...
public class TbFaultBillRefAlarmDaoImpl  extends UCMainDao  implements TbFaultBillRefAlarmDao {
	...
	public List<Map<String,Object>> queryBillRefAlarmId(String ids){
		QuoteBean quoteBean = this.getInWhereData(ids, QuoteBean.DataType.LONG);
		StringBuffer sql = new StringBuffer();
		sql.append(" select ");
		sql.append(" I_BILL_ID,I_ALARM_ID");
		sql.append(" from TB_FAULT_BILL_REF_ALARM");
		sql.append(" where I_ALARM_ID in ( ").append(quoteBean.getQuote()).append(")");
		
		return this.queryBySql(sql.toString(),quoteBean.getQuoteList(), this.getDomain());
	}
